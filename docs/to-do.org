...
melting, particularly in GC-rich or structurally complex templates, by
reducing secondary structures that can hinder primer binding and DNA
polymerase activity.
According to information from Takara, which provides DNA
polymerases, customers have reported improved amplification of GC-rich
templates by adding DMSO to reactions using PrimeSTAR MAX DNA Polymerase or
CloneAmp HiFi PCR Premix. The recommended concentration of DMSO for these
applications is between 2.5% and 5%.  However, I usually do not add DMSO to my
PCR reactions.


** temp_c (Union[float, int]) – Simulation temperature for dG (Celsius)
This is what I found:
When evaluating Gibbs free energy change (ΔG) for primer dimers in PCR, the simulation temperature typically used is:
